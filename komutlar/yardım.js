...
exports.run = async (client, message, args, member) => {
  ...
  if (
    ...
    let Genel = new Discord.MessageEmbed()
      ...
      .setDescription(
        client.commands
          .filter((cmd) => cmd.conf.kategori === "Genel")
          .map(
            (cmd) =>
              `:white_small_square: - **${prefix}${cmd.help.name}** ${cmd.help.description}`
          )
          .join("\n ")
      );
  ...
  if (args[0] === "Sahip" || args[0] === "sahip") {
    let Sahip = new Discord.MessageEmbed()
      ...
      .setDescription(
        client.commands
          .filter((cmd) => cmd.conf.kategori === "Sahip")
          .map(
            (cmd) =>
              `:white_small_square: - **${prefix}${cmd.help.name}** ${cmd.help.description}`
          )
  ...
  let embed = new Discord.MessageEmbed()
    ...
    .setColor("#FFFB05")
    .setDescription(
      `**Örnek Kullanım:** \`${prefix}yardım Kategori\` \n **Örnek:** \`${prefix}yardım Genel\``
    )
    .addField(
      "Kategoriler:",
      `
